...
# Instinct GPU partitioning enabled -- add a check
from megatron.core.parallel_state import (
    get_xcd_intra_gpu_parallel_group,
    get_xcd_intra_gpu_parallel_world_size,
    get_xcd_inter_gpu_parallel_group,
    get_xcd_inter_gpu_parallel_world_size,
    get_physical_gpu_idx,
)
...
# TODO:  Update CPU weight initialization for/with starscream/bb requirements
...
class TELinear22(te.pytorch.Linear22):
    """
    Wrapper for the Transformer-Engine's `Linear` layer.
...
class TELinear44(te.pytorch.Linear44):
    """
    Wrapper for the Transformer-Engine's `Linear` layer.
        ...
        *,
        ...
        if parallel_mode == "duplicated":
            ...
            if is_expert:
                ...
                #if False: # disable for now.
                if self.config.xcd_model_parallel_size <= 1:
                    extra_kwargs["intra_xcd_group"] = None
                    extra_kwargs["intra_xcd_size"] = 1
                    extra_kwargs["physical_gpu_offset"] = None
...
class TELayerNormColumnParallelLinear22(te.pytorch.LayerNormLinear22):
    """
    Wrapper for the Transformer-Engine's `LayerNormLinear` layer that combines
    layernorm and linear layers
    """
...
class TERowParallelLinear(TELinear):
    ...
    def __init__(
        ...
        *,
        ...
        # print('mlp for this tp rank:', get_tensor_model_parallel_rank(), input_size, output_size)
        super().__init__(
            input_size=input_size,
            output_size=output_size,
            parallel_mode="row",
...
class TERowParallelLinear22(TELinear22):
    """
    Wrapper for the Transformer-Engine's `Linear` layer but specialized similar
    to megatron's `RowParallelLinear` layer.
    """
    ...
    def __init__(
        ...
        *,
        ...
        # print('for this tp rank:', get_tensor_model_parallel_rank(), input_size, output_size)
        super().__init__(
            input_size=input_size,
            output_size=output_size,
            parallel_mode="bumblebee",
...
class TERowParallelLinear44(TELinear44):
    """
    Wrapper for the Transformer-Engine's `Linear` layer but specialized similar
    to megatron's `RowParallelLinear` layer.
    """
